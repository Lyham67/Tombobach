...
<<<<<<< HEAD
    ...
    if (siteContent.bigPrizes) {
        Object.keys(siteContent.bigPrizes).forEach(prizeNum => {
            const prize = siteContent.bigPrizes[prizeNum];
            
            // Image
            if (typeof prize === 'string') {
                const img = document.getElementById(`prize-img-${prizeNum}`);
                if (img) img.src = prize;
            } else if (prize.image) {
                const img = document.getElementById(`prize-img-${prizeNum}`);
                if (img) img.src = prize.image;
            }
            
            // Textes
            if (prize.title) {
                const title = document.querySelector(`.prize-card:nth-child(${prizeNum}) .prize-title`);
                if (title) title.textContent = prize.title;
            }
            if (prize.description) {
                const desc = document.querySelector(`.prize-card:nth-child(${prizeNum}) .prize-description`);
                if (desc) desc.textContent = prize.description;
            }
        });
    }
    
    // Images et textes des petits lots
    if (siteContent.smallPrizes) {
        Object.keys(siteContent.smallPrizes).forEach(lotNumber => {
            const prize = siteContent.smallPrizes[lotNumber];
            const img = document.querySelector(`.small-prize-image[data-lot="${lotNumber}"]`);
            const title = document.querySelector(`.small-prize-card:nth-child(${lotNumber}) .small-prize-title`);
...
function addBigPrizeEditors() {
    ...
    prizeCards.forEach((card, index) => {
        ...
        const img = card.querySelector('.prize-image');
        if (!img) return;
        ...
        btn.onclick = () => {
            ...
            input.onchange = (e) => {
                ...
                if (file) {
                    const reader = new FileReader();
                    reader.onload = (event) => {
                        if (!siteContent.bigPrizes) siteContent.bigPrizes = {};
                        if (typeof siteContent.bigPrizes[i] === 'string') {
                            siteContent.bigPrizes[i] = { image: event.target.result };
                        } else {
                            if (!siteContent.bigPrizes[i]) siteContent.bigPrizes[i] = {};
                            siteContent.bigPrizes[i].image = event.target.result;
                        }
                        img.src = event.target.result;
                    };
...
// Ajouter les éditeurs de texte pour tous les lots
function addTextEditors() {
    // Textes des 3 gros lots
    const prizeCards = document.querySelectorAll('.prize-card');
    prizeCards.forEach((card, index) => {
        const i = index + 1;
        if (i > 3) return;
        ...
        const desc = card.querySelector('.prize-description');
        
        if (title) {
            title.contentEditable = true;
            title.style.cursor = 'text';
            title.style.outline = '2px dashed #667eea';
            title.style.padding = '5px';
            title.addEventListener('blur', () => {
                if (!siteContent.bigPrizes) siteContent.bigPrizes = {};
                if (typeof siteContent.bigPrizes[i] === 'string') {
                    siteContent.bigPrizes[i] = { image: siteContent.bigPrizes[i] };
                }
                if (!siteContent.bigPrizes[i]) siteContent.bigPrizes[i] = {};
                siteContent.bigPrizes[i].title = title.textContent;
            });
        }
        ...
        if (desc) {
            ...
            desc.style.outline = '2px dashed #667eea';
            ...
            desc.addEventListener('blur', () => {
                ...
                if (!siteContent.bigPrizes[i]) siteContent.bigPrizes[i] = {};
                siteContent.bigPrizes[i].description = desc.textContent;
            });
        }
    });
    
    // Textes des petits lots
    const smallCards = document.querySelectorAll('.small-prize-card');
    smallCards.forEach((card, index) => {
        const i = index + 1;
        if (i > 10) return;
        
        const title = card.querySelector('.small-prize-title');
        ...
        if (title) {
            ...
            title.addEventListener('blur', () => {
                if (!siteContent.smallPrizes) siteContent.smallPrizes = {};
                if (typeof siteContent.smallPrizes[i] === 'string') {
                    siteContent.smallPrizes[i] = { image: siteContent.smallPrizes[i], title: title.textContent };
                } else {
                    if (!siteContent.smallPrizes[i]) siteContent.smallPrizes[i] = {};
                    siteContent.smallPrizes[i].title = title.textContent;
                }
            });
        }
    });
